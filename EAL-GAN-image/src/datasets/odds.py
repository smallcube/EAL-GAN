...
class ODDSADDataset(BaseADDataset):
    ...
    def __init__(self, root: str, dataset_name: str, n_known_outlier_classes: int = 0, ratio_known_normal: float = 0.0,
        ...
        semi_targets = torch.tensor(semi_targets)
        if semi_targets.shape[0]>=train_set.semi_targets.shape[0]:
            semi_targets = semi_targets[0:train_set.semi_targets.shape[0]]
        train_set.semi_targets[idx] = semi_targets   # set respective semi-supervised labels
