...
class Generator(nn.Module):
  def __init__(self, G_ch=64, G_depth=2, dim_z=128, bottom_width=4, resolution=128,
               ...
               G_param='SN', norm_style='bn', output_channel=3,
               **kwargs):
    ...
    self.output_layer = nn.Sequential(layers.bn(self.arch['out_channels'][-1],
                                                ...
                                                mybn=self.mybn),
                                    self.activation,
                                    self.which_conv(self.arch['out_channels'][-1], output_channel))
...
class Discriminator(nn.Module):
  def __init__(self, D_ch=64, D_wide=True, D_depth=2, resolution=128,
               ...
               D_init='N02', skip_init=False, D_param='SN', input_channel=3, **kwargs):
    ...
    # Prepare model
    # Stem convolution
    self.input_conv = self.which_conv(input_channel, self.arch['in_channels'][0])
    # self.blocks is a doubly-nested list of modules, the outer loop intended
    # to be over blocks at a given resolution (resblocks and/or self-attention)
    self.blocks = []
    for index in range(len(self.arch['out_channels'])):
    ...
    self.output_cat = self.which_linear(self.arch['out_channels'][-1], 1)
  ...
  def forward(self, x, y=None, mode=0):
    ...
    if mode==0:
      ...
      out_cat = F.sigmoid(self.output_cat(h))
      return out, out_cat
    else:
      out_cat = F.sigmoid(self.output_cat(h))
      return out_cat
