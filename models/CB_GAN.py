...
from .losses import loss_dis_fake, loss_dis_fake_V2, loss_dis_real, loss_gen, loss_fine_tuning, loss_fine_tuning_V2
...
class CB_GAN(nn.Module):
    ...
    def fit(self):
        ...
        for epoch in range(self.args.max_epochs):
            ...
            if self.args.print:
                print('Training for epoch %d: Train_AUC=%.4f train_Gmean=%.4f Test_AUC=%.4f  Test_Gmean=%.4f' % (epoch + 1, train_AUC, train_Gmean, test_auc, test_gmean))
    ...
    def train_one_epoch(self, data_x, data_y, z, y, min_label, netG, optimizerG, NetD_Ensemble, 
        ...
        for index in range(num_batches):
            ...
            for i in range(self.args.ensemble_num):
                ...
                loss1, real_loss_cat, real_weights, real_cat_weights = loss_dis_real(out_real_fake, out_real_categoy, real_y_selected, real_weights, real_cat_weights)
                real_loss = loss1+real_loss_cat
                
                #train on fake data
                output, out_fake_category = netD(generated_x, y.detach())
                loss2, loss_cat_fake, fake_weights, fake_cat_weights = loss_dis_fake_V2(output, out_fake_category, y, fake_weights, fake_cat_weights)
